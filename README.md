...
当前稳定版本：[v2.3.0](https://github.com/k7tmiz/A4-Memory/releases/tag/v2.3.0)
...
- 复习弹窗：自动复习（新增单词后立即打开，新词固定在第一张）/ 手动复习（复习整轮所有页），支持滑动标记、点击翻面；用户触发的弹层从实际控件展开并在关闭时回到来源
- 状态系统：已掌握 / 学习中 / 不会
- 轻量复习：按状态计算复习时间，记录页可聚合"待复习"
- 轮次类型：普通学习轮 / 已掌握复习 / 学习中复习 / 不会复习 / 待复习
- 学习记录：任务优先的今日复习与目标进度、紧凑轮次视图、状态视图、导出 CSV/PDF、生成复习轮；轮次详情和 A4 排版按需展开，桌面端和 Android 端会调用系统打印/保存为 PDF，Android JSON/CSV 导出会保存到下载目录
...
- 外观：学习、记录、设置常驻同一个无顶栏 App Shell，通过 History API 按底栏顺序横向切换；悬浮底栏使用滑动选中胶囊，离开学习视图时平滑收起「下一个单词」；设置采用账号、学习、发音、AI、更多五分类顶栏仪表盘，分类切换与记录页「轮次 / 状态」同为 Liquid Glass 滑动胶囊；记录突出今日任务与学习进度；桌面学习页 A4 水平居中，词书/工具与进度分居两侧；手机使用紧凑词书状态、纸面操作与下拉「更多工具」菜单（与记录页工具菜单同构），沉浸模式下纸面动态延伸至底栏上方；支持释义显示/隐藏、沉浸模式、减少动态效果、auto/light/dark 主题，以及经典、纸张绿、海蓝三套配色
- 交互与平台 UI：学习页纸面和背景只接受纵向页面手势，禁止页面横向偏移、双击和双指缩放，复习卡仍支持横向标记；沉浸模式可从纸面工具栏直接退出；应用拥有的提示、选择器和实时模型列表使用统一应用内 UI，打印、文件选择、外部浏览器、OAuth 和软键盘保留系统界面
- 备份：完整 JSON 导入/导出
- AI 生成词书：OpenAI / Gemini / DeepSeek / Custom；模型名可自由输入，也可使用当前会话内存中的 API Key 从服务商 `/models` 接口实时获取，不内置、缓存或回退模型列表
...
│   ├── records.css        # 记录页概览、轮次与状态视图
│   └── settings.css       # 设置页仪表盘与分类布局
├── data/words.js          # 内置词书
├── js/
│   ├── core/
│   │   ├── common.js     # 跨页共享业务逻辑
...
- 云端恢复会保留 A4 多页分页、轮次类型与语言；旧数据缺少页码时会按每页数量自动补分页
