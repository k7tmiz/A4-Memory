...
### `js/speech.js`
语音合成封装。Web/桌面端使用 SpeechSynthesis；Android Tauri 端通过全局 Tauri invoke 调用原生 `a4_android_speak`。当系统没有匹配语音时，支持在线 TTS 兜底（Microsoft Edge TTS / Google Translate TTS），由 `onlineTtsEnabled` / `onlineTtsProvider` 控制。
```javascript
window.A4Speech = {
