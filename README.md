...
- 发音：Web 端使用 SpeechSynthesis；Android Tauri 端通过原生 TextToSpeech 桥接发音，支持 en/es/ja/ko/pt/fr/de/it/eo；在线模式支持 Microsoft Edge / Google 翻译，浏览器直连优先，未及时开始播放时尝试同源代理、另一在线源和系统语音（不内置离线语音包）
