...
当前发布基线为 `2.3.0`。应用版本由 `package.json`、`package-lock.json`、`src-tauri/Cargo.toml`、`src-tauri/Cargo.lock`、`src-tauri/tauri.conf.json` 与 `js/updater.js` 共同声明，发版前必须保持一致。
...
│   └── settings.css       # 设置仪表盘、五分类顶栏与响应式布局
├── data/
...
所有宽度共用一条顶部分段导航，样式对齐记录页「轮次 / 状态」切换，并使用与底栏相同的 Liquid Glass 滑动胶囊。内容铺在页面底色上，不用浮起的卡片盒。宽度达到 760px 时，学习 / 发音 / AI / 更多分区改为两列排版；账号区仍通栏。所有宽度均复用 App Shell 的学习、记录、设置悬浮底栏。
