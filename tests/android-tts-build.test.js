...
describe("Android offline TTS build integration", () => {
  ...
  it("documents the arm64-only Android build and keeps release notes current", () => {
    ...
    assert.match(workflow, /从实际触发控件展开并在关闭时回到来源/)
    assert.match(workflow, /连续横向切换和统一悬浮底栏/)
    assert.match(workflow, /设置页采用统一页面式仪表盘/)
    assert.match(workflow, /记录页突出今日复习与进度/)
    assert.match(workflow, /a4-memory-v2\.1\.0-android\.apk/)
