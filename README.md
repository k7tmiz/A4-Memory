...
- Android 发音回退为系统 TextToSpeech，不再包含第三方离线 TTS 引擎、安装流程或额外权限
- Web / 桌面端继续使用 SpeechSynthesis；Android 端仅通过系统 TextToSpeech 桥接发音
- 已检查 TXT/CSV/JSON 词书导入、自动/手动复习、查词、设置备份入口、记录页导出入口
- 已覆盖恶意 HTML 词条渲染测试，避免导入内容在页面中变成真实标签或触发脚本
- 已刷新 Tauri、Vite、ESLint 与 Rust 依赖锁文件，并完成发布前漏洞扫描
...
- 发音：Web 端使用 SpeechSynthesis；Android Tauri 端通过原生 TextToSpeech 桥接发音，支持 en/es/ja/ko/pt/fr/de/it/eo（需系统已安装或启用 TTS 引擎，不内置离线语音包）
