...
- `index.html`（首页）
  ...
  - 响应式：控制区按操作频率分组；小屏压缩按钮尺寸，保证完整 A4 可视同时按钮可达
  - 交互一致性：设置按钮固定靠右（首页右上角）
  - 脚本顺序：`data/words.js` → `js/core/common.js` → `utils/storage/speech/settings/app`
- `records.html`（学习记录页）
  - 视图切换：轮次视图 / 状态视图
  - 顶部入口：返回背单词右侧为设置；导出区：导出 CSV、导出 PDF、清空记录（小屏为同一行的右侧大按钮）
  - 视觉一致性：与首页在同屏尺寸下的按钮尺寸与比例保持协调
  - 移动端：每轮卡片的操作按钮会缩放并换行以适配窄屏
