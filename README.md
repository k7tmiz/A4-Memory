...
一个纯前端背单词工具：把“单词随机写在 A4 上”作为学习载体。每新增 1 个单词都会自动打开复习弹窗：普通学习轮在多页时默认只复习当前页；用户主动点「复习本轮」才复习整轮全部页，减少“只会背新词”的错觉。支持学习记录、词书导入、打印/导出与发音。

## 功能

- A4 随机排版：单词随机落点，尽量避免重叠
- 复习本轮：作为独立入口复习整轮（同一轮内所有 A4 页面；默认打乱顺序，可恢复顺序；支持左右滑动/拖拽标记：右滑=已掌握、左滑=不会，手机/平板触摸与电脑鼠标拖拽均可；未达提交条件会自动平滑回中；支持快速甩动提交；复习完成默认自动关闭弹窗，可在设置关闭；可选开启“点击卡片翻面”，开启后：点单词发音、点空白翻面、背面点任意处翻回）
- 本轮去重：同一轮内按“同语言 + 同词同义（term+meaning）”去重，不重复出现相同词条
- 每轮上限 20–30 可调：当前 A4 写满后可在同一轮新增下一张 A4 继续学习（整轮去重），也可复习本轮或手动新建下一轮
...
- 多页 A4：
  - 普通学习轮：默认从 1 张 A4 开始；当前 A4 写满后可在同一轮追加第 2/3/... 张 A4
...
- 首页点“下一个单词” → 写入新词并自动弹出复习弹窗（新词固定在第一个；普通学习轮多页时默认只复习当前页）
- 需要复习时点“复习本轮” → 进入复习弹窗并标记状态（左右滑动/拖拽；未达提交条件会自动回中）
- 发音：普通模式与翻卡模式下，点击复习弹窗正面单词可发音（需在设置开启发音）
- 随时点“复习本轮”复习本轮全部单词（同一轮内所有 A4 页面），并标记学习状态
- 点“学习记录”：
  - 轮次视图：查看历史轮次、预览 A4、导出 CSV/PDF、跳回复习
  - 状态视图：按状态/待复习聚合查看，并可生成一轮进行复习
- 首页/记录页点“查词”：优先显示本地词书与学习记录状态，联网补充与西语变位按需展示；英文会优先显示中文释义并保留英文解释
- 查词结果可通过卡片顶部「加入当前轮」加入本轮：若当前 A4 已满会自动在本轮新增下一张 A4；加入后按现有“新增单词”流程自动打开复习弹窗（本次新增置顶，旧词按当前顺序）
