...
- 学习记录：轮次视图 + 状态视图（已掌握/学习中/不会/待复习），每轮显示轮次类型与状态统计
- 轮次类型：普通学习轮 / 已掌握复习轮 / 学习中复习轮 / 不会复习轮 / 待复习轮
- 状态生成轮：同一轮内可包含多张 A4（按每轮上限自动分页）
- 多页翻页：当一轮包含多张 A4 时，首页可翻页查看（Previous / Next）
- 导出：全局/单轮 CSV（含轮次类型与复习时间字段）、导出 PDF（1 轮 = 1 个 PDF，PDF 内每张 A4 占 1 页）
...
- 点“学习记录”按轮查看或按状态聚合查看；每轮可导出 CSV/PDF；导出 PDF 时每张 A4 占一页
- 在“学习记录”的状态视图里：可按状态/待复习生成一轮进行复习
