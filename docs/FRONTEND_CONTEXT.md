...
设置界面包含五个顶级类别，弹窗打开时显示「账号」：

1. **账号** — 登录、注册、重置密码、登录状态卡片与云同步（上传/下载数据）。登录状态卡片在宽度不超过 430px 时常显单词数、连续天数和当前轮，其余统计通过「更多学习统计」展开且每次打开设置默认折叠；平板和桌面宽度默认直接展示全部统计。
2. **学习** — 外观（主题模式）、学习目标（每日轮次/单词）、学习设置（每轮上限）和复习配置（复习间隔、翻面、持续背书）。
3. **发音** — 朗读开关、发音方式（在线 TTS / 离线 TTS / 系统语音）和在线发音源；桌面端和 Android 应用均显示离线 TTS。选中离线 TTS 时展开「离线语音包」分区：列出可信远端清单（失败时使用内置清单）与已安装项，按语言显示下载/删除/试听按钮和默认语音选择。
4. **AI** — 自定义 API 配置、模型选择和词书生成。
5. **更多** — 包含联网补充（在线查词）、数据管理和版本信息；版本信息仅在 Tauri 桌面端与 Android 应用中显示。

移动端使用跟随主题的统一分段式顶部导航，内容按单列手风琴卡片排列。宽度达到 760px 时，导航位于左侧，内容使用响应式两列手风琴布局；账号面板横跨整个内容区。
