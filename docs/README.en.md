...
Current stable release: [v2.3.0](https://github.com/k7tmiz/A4-Memory/releases/tag/v2.3.0)
...
- Appearance: Study, Records, and Settings stay mounted in one headerless App Shell and move horizontally in dock order through the History API; the floating dock uses a sliding active indicator and smoothly collapses “Next Word” away from Study; Settings uses a five-category top switch for Account, Study, Pronunciation, AI, and More, with the same Liquid Glass sliding capsule as the Records Rounds/Status control; Records emphasizes today’s tasks and learning progress; on desktop the A4 sheet is horizontally centered with wordbook/tools and progress fixed on either side; phones use a compact wordbook row, paper actions, and a dropdown “more tools” menu matching the Records tools menu, while immersive mode extends the sheet dynamically to the space above the dock; includes meaning toggle, immersive mode, reduced-motion support, auto/light/dark theme, and Classic, Paper Green, and Ocean palettes
- Interaction and platform UI: the Study sheet and page background accept vertical page gestures only, preventing horizontal page drift, double-tap zoom, and pinch zoom, while review cards retain horizontal marking gestures; immersive mode can be exited directly from the paper toolbar; app-owned notices, pickers, and live model choices use consistent in-app UI, while printing, file selection, the external browser, OAuth, and the soft keyboard retain their system UI
- Backup: full JSON import/export
- AI wordbook generator: OpenAI / Gemini / DeepSeek / Custom; model IDs remain freely editable and can be fetched live from the provider's `/models` endpoint with the in-memory API key, without a built-in, cached, or fallback model catalog
