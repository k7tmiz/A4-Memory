...
describe("Android offline TTS build integration", () => {
  ...
  it("documents the arm64-only Android build and keeps release notes current", () => {
    ...
    assert.match(workflow, /桌面学习页 A4 视口水平居中/)
    assert.match(workflow, /手机「更多工具」改为与记录页同构的下拉菜单/)
    assert.match(workflow, /查词弹窗与词书选择面板视觉统一/)
    assert.match(workflow, /底部面板 `sheet` 动效按面板高度滑入/)
    assert.match(workflow, /a4-memory-v2\.2\.0-android\.apk/)
