...
- Interaction and platform UI: the Study sheet and page background accept vertical page gestures only, preventing horizontal page drift, double-tap zoom, and pinch zoom, while review cards retain horizontal marking gestures; immersive mode can be exited directly from the paper toolbar; app-owned notices, pickers, and common-model choices use consistent in-app UI, while printing, file selection, the external browser, OAuth, and the soft keyboard retain their system UI
